 ...
 *
 *
 * Maps a Unicode character to its corresponding virtual key code and modifier mask
...
KeyboardInfo GetKeyboardInfo()
{
    ...
    CFDataRef layoutData = TISGetInputSourceProperty(layoutRef, kTISPropertyUnicodeKeyLayoutData);
 ...
 *
 *
 * Simulates a key press or release event for a given key code and modifier flags.
 *
...
{
    snprintf(LastErrorMessage, sizeof(LastErrorMessage),
             "Error performing KeyPress with key=%d flags=%llu",
             key, flags);
...
{
    snprintf(LastErrorMessage, sizeof(LastErrorMessage),
             "Error performing KeyRelease with key=%d flags=%llu",
             key, flags);
...
{
    snprintf(LastErrorMessage, sizeof(LastErrorMessage),
             "Error performing KeyPressAndRelease with key=%d flags=%llu",
             key, flags);
 ...
 *
 *
 * Presses or releases modifier keys (Shift, Option) as needed for the current and next key.
 *
...
int SetMods(CGEventFlags *flags, UInt32 mods, UInt32 modsNext, bool keyDown)
{
    ...
    for (int i = 0; i < 2; i++)
    {
        ...
        if ((mods & m.mask) != 0) // mod mask has bits present
        {
            *flags |= m.flag; // set flags
            ...
            if (!keyDown) // key up
            {
                if ((modsNext & m.mask) == 0) // next key's mod mask does not have bits present
                {
                    KeyRelease(m.key, 0);
                    counter++;
                }
    ...
    snprintf(LastErrorMessage, sizeof(LastErrorMessage), "Unable to perform SetMods with flags=%llu mods=%x", *flags, mods);
 ...
 *
 *
 * Types a string by simulating key presses/releases for each character, handling modifiers,
 * delays, and tab-to-space conversion.
 *
 ...
 * @return            1 on success, 0 on failure.
 */
int TypeStr(const char *str, int modPressDur, int keyPressDur, int keyDelay, int tabSize)
{
    int len = strlen(str);
    int last = len - 1;
    int errCount = 0;
